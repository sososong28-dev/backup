...
- 首页不放入口，只能通过隐藏地址访问。
- 页面支持 `1×2` 和 `2×5` 两种布局；`1×2` 每屏 2 张，`2×5` 每屏 10 张，并带有清晰序号。
- 无 `voter` 参数时为主页面：
...
4. 使用顶部 `1×2` / `2×5` 按钮调整页面布局；浏览器会记住当前布局选择。

5. 点击 `新增外发端`，输入外发端名称和使用次数；使用次数留空则不限。生成的链接会绑定当前项目和当前产品标签。

6. 系统生成带 `?voter=...` 的专属链接。

7. 把不同链接分别发给不同评审人员。

8. 外发端在自己的链接中投票和留言，最后点击红色 `提交确认` 按钮完成确认。

9. 主页面查看每张图的汇总结果，并通过 `导出全部` 一键导出完整统计结果。
10. 主页面可通过 `日志` 查看和单独导出操作日志。
11. 主页面可通过 `分发设置` 修改每个分发端的使用次数。
12. 主页面可在每张图片下方填写一句话说明，分发页面自动同步显示。
